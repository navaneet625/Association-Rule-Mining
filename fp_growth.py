...
def updateTree(itemset, FPTree, HeaderTable, count):
    if itemset[0] in FPTree.children:
        ...
        if HeaderTable[itemset[0]][1] == None:
            ...
            Test_Node = HeaderTable[itemset[0]][1]
            Target_Node = FPTree.children[itemset[0]]
...
def fpw_lib(data,s):
    st=time.time()
    patterns = pyfpgrowth.find_frequent_patterns(data,s) 
    en = time.time()
    print("Time Taken is : ",en-st)
    print()
    print("All frequent itemsets:")
    for f in patterns:
        print(f)

def fpw_scratch(data,s):

    start = time.time()
    FPtree, HeaderTable = create_FPTree(data, s)
    frequent_itemset = []

    Mine_Tree(FPtree, HeaderTable,s, set([]), frequent_itemset)
    end = time.time()
...
def main():
    ...
    print("----------fp growth frequent itemset using libiray------------")
    print()
    fpw_lib(data,s)
    print()


    print("----------fp growth frequent itemset from scratch ------------")
    print()
    fpw_scratch(data,s)
    print()
